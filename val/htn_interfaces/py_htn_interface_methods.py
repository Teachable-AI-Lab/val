...
def dict_to_operators_and_methods(operator_dict_list: list) -> List[Operator]:
    domain = defaultdict(list)
    task_descriptions = {}

    for operator_dict in operator_dict_list:
        if operator_dict['type']=='operator':
            ...
            for precondition_dict in operator_dict['preconditions']:
                ...
                if precondition_dict['type'] == 'bind':
                    raise NotImplementedError("Not implemented yet")
            subtasks = []
            for subtask in operator_dict['subtasks']:
                if isinstance(subtask, list):
                    subtask_list = []
                    for task in subtask:
                        head = (task['name'])
                        primitive = task['primitive']
                        subtask_list.append(Task(head=head, primitive=primitive))
                    subtasks.append(subtask_list)
                else:
                    head = (subtask['name'])
                    primitive = subtask['primitive']
                    subtasks.append(Task(head=head, primitive=primitive))
                    

            # TODO remove effects from operator, it will just return the operator name and args
            new_method = Method(head=head, preconditions=AND(*flatten(preconditions)), subtasks=subtasks)
            key = f"{ new_method.name }/{ len(subtasks) }"
            domain[key].append(new_method)
            task_descriptions[key] = operator_dict['description']
...
class PyHtnInterface(AbstractHtnInterface):
    ...
    def __init__(self, agent):
        ...
        self.domain, self.task_description = self.agent.env.get_actions()
